...
export function registerExpenseConfirmTool(server, userId) {
    server.registerTool('expense.confirm', {
        title: 'Confirmar gasto (persistir)',
        description: 'Valida y persiste un borrador como Expense. Clasifica y devuelve estatus de sobres (envelopes).',
        ...
        if (!draft)
            return toolError(`No existe borrador draftId=${args.draftId}`);
