...
import { buildExpenseClassificationMessages, type LlmExpenseClassificationJson } from './prompts.js';
import type {
...
export type ClassifyExpenseInput = {
  ...
  rawText?: string | null;
  /**
   * Si ya se conoce el merchant (ej: import), se puede pasar directo.
   * Si no, se intentará resolver por alias/nameCanonical.
   */
...
function buildHaystack(input: ClassifyExpenseInput): string {
  return [input.merchantRaw, input.description ?? '', input.rawText ?? '']
    .join(' ')
    .trim();
}

function matchesRule(matchType: RuleMatchType, pattern: string, haystack: string): boolean {
 ...
 */
export async function classifyExpense(input: ClassifyExpenseInput): Promise<ExpenseClassificationResult> {
  const userId = normalizeText(input.userId);
  const merchantRaw = normalizeText(input.merchantRaw);
  if (!userId) throw new Error('classifyExpense: userId requerido');
  if (!merchantRaw) throw new Error('classifyExpense: merchantRaw requerido');
  ...
  let resolvedMerchantId: string | null = input.merchantId ?? null;
  if (alias?.merchantId) resolvedMerchantId = alias.merchantId;
  ...
  for (const rule of rules) {
    if (matchesRule(rule.matchType as RuleMatchType, rule.pattern, haystack)) {
      return {
  ...
  // 3) Merchant default
  let merchant:
    | { id: string; nameCanonical: string; defaultCategoryId: string | null }
    | null = null;
  ...
  if (resolvedMerchantId) {
    ...
    if (merchant?.id) resolvedMerchantId = merchant.id;
  }
  ...
  const llm = await openrouterChatJson<LlmExpenseClassificationJson>(messages, {
    signal: input.signal
  });
