...
export function registerExpenseCorrectTool(server: McpServer, userId: string) {
  server.registerTool(
    'expense.correct',
    {
      title: 'Corregir gasto (persistido)',
      description:
        'Actualiza la categoría (categoryId) de un Expense ya persistido y aprende una heurística (Merchant.defaultCategoryId o ClassificationRule).',
    ...
    async (args) => {
      ...
      if (!expense) return toolError(`No existe expenseId=${args.expenseId} para este usuario`);

      const category = await financeDb.category.findFirst({
        where: { id: args.categoryId, userId, isActive: true },
        select: { id: true }
      });
      if (!category) return toolError(`No existe categoryId=${args.categoryId} (activa) para este usuario`);
