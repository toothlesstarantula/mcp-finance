...
const { financeDbMock, classifyExpenseMock, budgetStatusMock, reportMonthlyMock } = vi.hoisted(() => {
  const financeDbMock = {
    expense: {
      create: vi.fn(),
      findMany: vi.fn(),
  ...
  return { financeDbMock, classifyExpenseMock, budgetStatusMock, reportMonthlyMock };
});
...
describe('mcp tools (smoke)', () => {
  ...
  it('budget.status y report.monthly', async () => {
    budgetStatusMock.mockResolvedValue([
      { name: 'Comida', period: 'MONTHLY', spentCents: 10, budgetCents: 20, remainingCents: 10 }
    ]);
    ...
    reportMonthlyMock.mockResolvedValue({
      ...
      topMerchants: [{ merchantId: null, merchantRaw: 'OXXO', totalCents: 100 }]
    });

    const server = createMcpServerForUser('u1');
    const budgetTool = (server as any)._registeredTools['budget.status'].handler;
    const reportTool = (server as any)._registeredTools['report.monthly'].handler;
  ...
  it('expense.correct (persist + learning rule contains)', async () => {
    ...
    financeDbMock.classificationRule.aggregate.mockResolvedValue({ _min: { priority: 3 } });
    ...
    expect(res.structuredContent).toMatchObject({
      expenseId: 'exp_2',
      categoryId: 'cat_food',
      learned: { type: 'RULE_CONTAINS', ruleId: 'r_1', priority: 2, pattern: 'PUESTO DE TACOS' }
    });
